...
Effect.Scale = Class.create(Effect.Base, {
  initialize: function (element, percents) {
    this.element = $(element);
    if (!this.element) throw Effect._elementDoesNotExistError;
    var options = Object.extend(
      {
  ...
  setup: function () {
    ...
    this.factor = [
      (this.options.scaleTo[0] - this.options.scaleFrom[0]) / 100,
      (this.options.scaleTo[1] - this.options.scaleFrom[1]) / 100,
    ];
  ...
  update: function (position) {
    var currentScale0 =
      this.options.scaleFrom[0] / 100.0 + this.factor[0] * position;
    var currentScale1 =
      this.options.scaleFrom[1] / 100.0 + this.factor[1] * position;
    this.setDimensions(
      this.dims[0] * currentScale0,
      this.dims[1] * currentScale1,
    );
  },
  finish: function (position) {
...
Effect.Puff = function (element) {
  ...
  return new Effect.Parallel(
    [
      new Effect.Scale(element, [200, 200], {
        sync: true,
    ...
    Object.extend(
      {
        ...
        afterFinishInternal: function (effect) {
          new Effect.Scale(effect.element, [1, 1], {
            duration: 0.3,
            scaleFromCenter: true,
            scaleX: false,
            scaleContent: false,
    ...
    Object.extend(
      {
        scaleContent: false,
        scaleX: false,
        scaleFrom: window.opera ? [0, 0] : [1, 1],
        scaleMode: {
          originalHeight: elementDimensions.height,
          originalWidth: elementDimensions.width,
        },
...
Effect.Squish = function (element) {
  return new Effect.Scale(element, window.opera ? [1, 1] : [0, 0], {
    restoreAfterFinish: true,
    beforeSetup: function (effect) {
    ...
    afterFinishInternal: function (effect) {
      new Effect.Parallel(
        [
          ...
          new Effect.Scale(effect.element, [100, 100], {
            scaleMode: {
              originalHeight: dims.height,
              originalWidth: dims.width,
            },
            sync: true,
            scaleFrom: window.opera ? [1, 1] : [0, 0],
...
Effect.Shrink = function (element) {
  ...
  return new Effect.Parallel(
    [
      ...
      new Effect.Scale(element, window.opera ? [1, 1] : [0, 0], {
        sync: true,
        transition: options.scaleTransition,
        restoreAfterFinish: true,
      }),
